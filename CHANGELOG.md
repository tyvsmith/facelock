...
- **A synthetic camera for the camera-required test tiers** (#139): `just test-arch-loopback`
  ...
  frame. `just release-preflight` requires its record at HEAD alongside the real-camera record,
  and CI runs it on every pull request (`loopback-e2e`). A model-backed contract test pins the
...
- The release matrix now pins the APT signing key fingerprint, uid, and
  expiry against the published docs; the APT publisher refuses to sign with
  an imported key whose fingerprint, uid, or expiry differs from that pin,
  naming the mismatch. The apt client lane installs the keyring at the
  documented `/usr/share/keyrings` path, and its v0.1.4-client replay uses
  the `/etc/apt/keyrings` path that release actually shipped (#346).
...
- **The release workflow's container steps run under the shell they are written
  in**: the second tagged run built both Debian packages and then threw the
  result away, because the step that reads the package version back died with
