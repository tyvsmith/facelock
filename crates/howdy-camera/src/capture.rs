use howdy_core::config::DeviceConfig;
...
impl<'a> Camera<'a> {
    ...
    pub fn open(config: &DeviceConfig) -> Result<Camera<'static>> {
        let dev = Device::with_path(&config.path)
            .map_err(|e| HowdyError::Camera(format!("failed to open {}: {e}", config.path)))?;

        // Verify VIDEO_CAPTURE capability
        let caps = dev.query_caps().map_err(|e| {
            HowdyError::Camera(format!("failed to query caps for {}: {e}", config.path))
        })?;
        if !caps
            .capabilities
            .contains(v4l::capability::Flags::VIDEO_CAPTURE)
        {
            return Err(HowdyError::Camera(format!(
        ...
        let formats = dev
            .enum_formats()
            .map_err(|e| HowdyError::Camera(format!("failed to enum formats: {e}")))?;
        ...
        let selected_fourcc = preferred
            ...
            .ok_or_else(|| {
                HowdyError::Camera(format!("{}: no supported formats", config.path))
            })?;
        ...
        let mut fmt = dev
            .format()
            .map_err(|e| HowdyError::Camera(format!("failed to get format: {e}")))?;
        fmt.fourcc = selected_fourcc;
        fmt.width = max_w;
        fmt.height = max_h;
        let fmt = dev
            .set_format(&fmt)
            .map_err(|e| HowdyError::Camera(format!("failed to set format: {e}")))?;
        ...
        let stream = Stream::with_buffers(&dev, Type::VideoCapture, 4)
            .map_err(|e| HowdyError::Camera(format!("failed to create stream: {e}")))?;
    ...
    fn capture_rgb(&mut self) -> Result<(Vec<u8>, u32, u32)> {
        let (buf, _meta) = self
            .stream
            .next()
            .map_err(|e| HowdyError::Camera(format!("capture failed: {e}")))?;
        ...
        let rgb: Vec<u8> = match self.format.as_str() {
            ...
            "MJPG" => {
                ...
                .decode()
                .map_err(|e| HowdyError::Camera(format!("MJPG decode failed: {e}")))?;
        ...
        if height > self.height {
            let img = image::RgbImage::from_raw(width, height, rgb).ok_or_else(|| {
                HowdyError::Camera("failed to create image for resize".into())
            })?;
        ...
        if self.rotation != 0 {
            let img = image::RgbImage::from_raw(width, height, rgb).ok_or_else(|| {
                HowdyError::Camera("failed to create image for rotation".into())
            })?;
