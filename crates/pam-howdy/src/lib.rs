// PAM module for howdy-rust face authentication.
//
// STRICT dependency rule: libc, toml, serde ONLY. No howdy-core.
...
// Default config path
const DEFAULT_CONFIG_PATH: &str = "/etc/howdy/config.toml";
const DEFAULT_SOCKET_PATH: &str = "/run/howdy/howdy.sock";
...
fn load_config() -> Result<PamConfig, String> {
    let config_path = std::env::var("HOWDY_CONFIG")
...
"#;
        let config: PamConfig = toml::from_str(toml_str).unwrap();
        assert_eq!(config.daemon.socket_path, "/run/howdy/howdy.sock");
        assert!(config.security.disabled);
        assert!(!config.security.abort_if_ssh);
        assert!(!config.security.abort_if_lid_closed);
