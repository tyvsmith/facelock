...
use facelock_core::types::{Frame, Point2D};
...
pub fn compute_affine_matrix(src: &[Point2D; 5]) -> [[f32; 3]; 2] {
    ...
    let det_uv = (u[0][0] * u[1][1] - u[0][1] * u[1][0]) * (v[0][0] * v[1][1] - v[0][1] * v[1][0]);
    let d = if det_uv < 0.0 {
        [1.0f32, -1.0]
    } else {
        [1.0f32, 1.0]
...
fn eigenvectors_2x2_sym(a: f32, b: f32, _c: f32, lambda1: f32, lambda2: f32) -> [[f32; 2]; 2] {
...
pub fn align_face(frame: &Frame, landmarks: &[Point2D; 5]) -> Result<AlignedFace> {
    ...
    let det = a * d - b * c;
    if det.abs() < 1e-10 {
        return Err(FacelockError::Alignment(
            ...
            for c in 0..3 {
                let get_pixel = |px: i32, py: i32| -> f32 {
                    if px >= 0 && px < w && py >= 0 && py < h {
                        frame.rgb[(py as usize * frame.width as usize + px as usize) * 3 + c] as f32
                    } else {
                        0.0 // black padding
                    }
                };
