...
impl FaceEmbedder {
    ...
    pub fn load(model_path: &Path, threads: u32) -> Result<Self> {
        ...
        let session = Session::builder()
            .map_err(|e| FacelockError::Embedding(format!("Failed to create session builder: {e}")))?
            .with_optimization_level(GraphOptimizationLevel::Level3)
            .map_err(|e| FacelockError::Embedding(format!("Failed to set optimization level: {e}")))?
