...
impl FaceEmbedder {
    ...
    pub fn load(model_path: &Path, threads: u32, execution_provider: &str) -> Result<Self> {
        crate::provider::ensure_runtime_loaded(execution_provider)
        ...
        let builder = Session::builder()
            ...
            .map_err(|e| {
                FacelockError::Embedding(format!("Failed to set optimization level: {e}"))
            })?
            .with_intra_threads(threads as usize)
            .map_err(|e| FacelockError::Embedding(format!("Failed to set intra threads: {e}")))?;

        let mut builder = crate::provider::register_execution_provider(builder, execution_provider)
            .map_err(|e| {
                FacelockError::Embedding(format!("Failed to set execution provider: {e}"))
            })?;

        let session = builder.commit_from_file(model_path).map_err(|e| {
            FacelockError::Embedding(format!(
    ...
    pub fn embed(&mut self, aligned: &AlignedFace) -> Result<FaceEmbedding> {
        ...
        let input_value = Tensor::from_array((shape.as_slice(), data.into_boxed_slice()))
            .map_err(|e| FacelockError::Embedding(format!("Failed to create input tensor: {e}")))?;

        let outputs = self
            .session
            .run(ort::inputs![input_value])
            .map_err(|e| FacelockError::Embedding(format!("Inference failed: {e}")))?;

        let (_shape, embedding_data) = outputs[0]
            .try_extract_tensor::<f32>()
            .map_err(|e| FacelockError::Embedding(format!("Failed to extract embedding: {e}")))?;
