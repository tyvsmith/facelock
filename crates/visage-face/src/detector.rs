...
use visage_core::types::{BoundingBox, Detection, Frame, Point2D};
...
impl FaceDetector {
    ...
    pub fn detect(&mut self, frame: &Frame) -> Result<Vec<Detection>> {
        ...
        let input_value = Tensor::from_array((shape.as_slice(), tensor_data.into_boxed_slice()))
            .map_err(|e| VisageError::Detection(format!("Failed to create input tensor: {e}")))?;

        let outputs = self
            .session
            .run(ort::inputs![input_value])
            .map_err(|e| VisageError::Detection(format!("Inference failed: {e}")))?;
        ...
        for (stride_idx, &stride) in STRIDES.iter().enumerate() {
            ...
            let (_score_shape, scores) = outputs[score_idx]
                ...
                .map_err(|e| {
                    VisageError::Detection(format!("Failed to extract scores tensor: {e}"))
                })?;
            let (_bbox_shape, bboxes) = outputs[bbox_idx]
                .try_extract_tensor::<f32>()
                .map_err(|e| {
                    VisageError::Detection(format!("Failed to extract bbox tensor: {e}"))
                })?;
            let (_kps_shape, landmarks_data) = outputs[kps_idx]
                .try_extract_tensor::<f32>()
                .map_err(|e| {
                    VisageError::Detection(format!("Failed to extract landmarks tensor: {e}"))
                })?;
