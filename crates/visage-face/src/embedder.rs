...
impl FaceEmbedder {
    ...
    pub fn embed(&mut self, aligned: &AlignedFace) -> Result<FaceEmbedding> {
        ...
        let input_value = Tensor::from_array((shape.as_slice(), data.into_boxed_slice()))
            .map_err(|e| VisageError::Embedding(format!("Failed to create input tensor: {e}")))?;

        let outputs = self
            .session
            .run(ort::inputs![input_value])
            .map_err(|e| VisageError::Embedding(format!("Inference failed: {e}")))?;

        let (_shape, embedding_data) = outputs[0]
            .try_extract_tensor::<f32>()
            .map_err(|e| VisageError::Embedding(format!("Failed to extract embedding: {e}")))?;
