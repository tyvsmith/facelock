...
use visage_camera::capture::Camera;
use visage_camera::device::{is_ir_camera, validate_device};
...
#[derive(Parser)]
#[command(name = "visage-bench", about = "Benchmark and calibration tool for visage")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
fn load_config() -> Result<Config> {
    Config::load().context(
        "Failed to load config. Set VISAGE_CONFIG env var or ensure /etc/visage/config.toml exists.",
    )
}
...
fn open_camera(config: &Config) -> Result<Camera<'static>> {
    ...
    Camera::open(&config.device).with_context(|| format!("Failed to open camera ({path_display})"))
}
...
fn cmd_cold_auth() -> Result<()> {
    ...
    if embeddings.is_empty() {
        bail!(
            "No enrolled faces for user '{}'. Enroll first with `visage add`.",
            user
        );
    }
...
fn cmd_report() -> Result<()> {
    ...
    println!("# Visage Benchmark Report");
    println!();
    println!("## Environment");
    ...
    if has_enrolled {
        println!("- Run `visage-bench calibrate` for threshold sweep");
    } else {
...
fn find_best_match(
    faces: &[(visage_core::types::Detection, FaceEmbedding)],
...
mod tests {
    ...
    fn test_find_best_match_above_threshold() {
        let emb = [0.1f32; 512];
        let faces = vec![(
            visage_core::types::Detection {
                bbox: visage_core::types::BoundingBox {
                    x: 0.0,
                    y: 0.0,
                    width: 100.0,
                    height: 100.0,
                ...
                confidence: 0.9,
                landmarks: [visage_core::types::Point2D { x: 0.0, y: 0.0 }; 5],
            },
            emb,
        )];
