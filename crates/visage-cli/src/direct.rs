...
pub fn authenticate(config: &Config, user: &str) -> anyhow::Result<bool> {
    ...
    // Reuse the shared authenticate function from visage-daemon's auth module.
    // Since it returns DaemonResponse (which is an IPC type), we map it here.
    // This avoids duplicating the auth loop.
    let response = visage_daemon_auth::authenticate(
        &mut camera,
        &mut engine,
        &store,
        config,
...
pub fn enroll(
    ...
    let response = visage_daemon_enroll::enroll(
        &mut camera,
        &mut engine,
        &store,
        config,
...
//
// Actually, since visage-daemon is a binary crate, we can't import from it.
// The auth and enroll modules use types from visage-core's traits, and the concrete
