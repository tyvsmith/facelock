...
use howdy_core::types::{BoundingBox, Detection, Frame, Point2D};
...
impl FaceDetector {
    ...
    pub fn load(model_path: &Path, confidence: f32, nms: f32) -> Result<Self> {
        let session = Session::builder()
            .map_err(|e| HowdyError::Detection(format!("Failed to create session builder: {e}")))?
            .with_optimization_level(GraphOptimizationLevel::Level3)
            .map_err(|e| HowdyError::Detection(format!("Failed to set optimization level: {e}")))?
            .with_intra_threads(4)
            .map_err(|e| HowdyError::Detection(format!("Failed to set intra threads: {e}")))?
            .commit_from_file(model_path)
            .map_err(|e| {
                HowdyError::Detection(format!(
    ...
    pub fn detect(&mut self, frame: &Frame) -> Result<Vec<Detection>> {
        ...
        let input_value = Tensor::from_array((shape.as_slice(), tensor_data.into_boxed_slice()))
            .map_err(|e| HowdyError::Detection(format!("Failed to create input tensor: {e}")))?;

        let outputs = self
            .session
            .run(ort::inputs![input_value])
            .map_err(|e| HowdyError::Detection(format!("Inference failed: {e}")))?;
        ...
        for (stride_idx, &stride) in STRIDES.iter().enumerate() {
            ...
            let (_score_shape, scores) = outputs[score_idx]
                .try_extract_tensor::<f32>()
                .map_err(|e| {
                    HowdyError::Detection(format!("Failed to extract scores tensor: {e}"))
                })?;
            let (_bbox_shape, bboxes) = outputs[bbox_idx]
                .try_extract_tensor::<f32>()
                .map_err(|e| {
                    HowdyError::Detection(format!("Failed to extract bbox tensor: {e}"))
                })?;
            let (_kps_shape, landmarks_data) = outputs[kps_idx]
                .try_extract_tensor::<f32>()
                .map_err(|e| {
                    HowdyError::Detection(format!("Failed to extract landmarks tensor: {e}"))
                })?;
...
mod tests {
    ...
    fn iou_identical_boxes() {
        ...
        assert!((iou - 1.0).abs() < 1e-5, "identical boxes should have IoU 1.0, got {iou}");
    }
    ...
    fn iou_non_overlapping() {
        ...
        assert!(iou.abs() < 1e-5, "non-overlapping boxes should have IoU 0.0, got {iou}");
    }
    ...
    fn letterbox_square_image_no_padding() {
        ...
        assert!((scale - 1.0).abs() < 1e-5, "square image scale should be 1.0, got {scale}");
        assert!(pad_x.abs() < 1e-5, "square image should have no x padding, got {pad_x}");
        assert!(pad_y.abs() < 1e-5, "square image should have no y padding, got {pad_y}");
    }
    ...
    fn letterbox_wide_image_vertical_padding() {
        ...
        assert!((scale - 0.5).abs() < 1e-5, "wide image scale should be 0.5, got {scale}");
        assert!(pad_y > 0.0, "wide image should have vertical padding, got {pad_y}");
    }
}
