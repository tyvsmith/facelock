...
mod tests {
    ...
    fn sharpness_score_uniform_is_low() {
        ...
        assert!(score < 0.1, "uniform region should have low sharpness: {score}");
    }
    ...
    fn angle_diversity_single_embedding() {
        let emb = [0.5f32; 512];
        assert!(check_angle_diversity(&[emb]), "single embedding should pass diversity check");
    }
    ...
    fn quality_hints() {
        ...
        assert_eq!(
            quality_hint(&low_size),
            Some("Move closer to the camera")
        );
    }
}
