...
pub fn auto_detect_device() -> Result<DeviceInfo> {
    ...
    devices
        ...
        .or(devices.first())
        .cloned()
        .ok_or_else(|| VisageError::Camera("no video devices found".into()))
}

fn query_device(path: &str) -> Result<DeviceInfo> {
    let dev =
        Device::with_path(path).map_err(|e| VisageError::Camera(format!("{path}: {e}")))?;

    let caps = dev
        .query_caps()
        .map_err(|e| VisageError::Camera(format!("{path}: failed to query caps: {e}")))?;
