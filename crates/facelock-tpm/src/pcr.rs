    ...
    pub fn verify_against_baseline(
            ...
            baseline.values.iter().zip(current.iter())
        {
            if baseline_digest != current_digest {
                warn!(pcr_index = idx, "PCR value mismatch against baseline");
                return Ok(false);
            }
        }
...
#[cfg(feature = "tpm")]
pub(crate) fn pcr_index_to_slot(index: u32) -> Result<tss_esapi::structures::PcrSlot> {
    ...
    match index {
        ...
        _ => Err(FacelockError::Tpm(format!(
            "invalid PCR index: {index} (must be 0-23)"
        ))),
    }
}
