...
impl QuirksDb {
    ...
    pub fn load() -> Self {
        ...
        for dir in &[
            "/usr/share/facelock/quirks.d",
            "/etc/facelock/quirks.d",
        ] {
...
mod tests {
    ...
    fn name_matches_case_insensitive() {
        ...
        assert!(name_matches("(?i)integrated.*infrared", "Integrated Infrared Camera"));
        // Both keywords covered by searching for just the common part
        assert!(name_matches("(?i)integrated.*in", "Integrated Infrared Camera"));
    }
...
"#;
    ...
    fn quirks_load_dir_nonexistent() {
        ...
        assert!(db.quirks.is_empty(), "nonexistent dir should load zero quirks");
    }
