...
**Phase**: 4 (Interfaces) | **Crate**: facelock-cli | **Depends on**: 01 (IPC protocol) | **Parallel with**: 06
...
### `facelock enroll [--user USER] [--label LABEL]`
...
### `facelock remove <ID> [--user USER] [-y|--yes]`
...
### `facelock clear [--user USER] [-y|--yes]`
...
5. PAM: `/lib/security/pam_facelock.so` exists? `/etc/pam.d/sudo` contains facelock?
