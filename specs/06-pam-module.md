...
**Phase**: 4 (Interfaces) | **Crate**: pam-facelock | **Depends on**: 01 (IPC protocol) | **Parallel with**: 07
...
**STRICT**: `libc`, `toml`, `serde` ONLY. No facelock-core, no ort, no v4l, no rusqlite.

The PAM module must stay tiny (~200KB). It reads a minimal config subset and speaks the IPC protocol directly (reimplemented in ~50 lines, not imported from facelock-core).
...
fn identify(pamh: *mut libc::c_void) -> libc::c_int {
    ...
    // ALL outcomes logged to syslog: pam_facelock(<service>): <result> for user <username>
}
```
...
- Build verification: `cargo build -p pam-facelock --release`
- Symbol check: `nm -D target/release/libpam_facelock.so | grep pam_sm_authenticate`
- Size check: `stat -c%s target/release/libpam_facelock.so` (should be < 500KB)
- Dependency check: `ldd target/release/libpam_facelock.so` (minimal deps)
...
fn log_auth(service: &str, result: &str, user: &str) {
    ...
    // Format: pam_facelock(sudo): success for user ty
    unsafe {
        let ident = std::ffi::CString::new("pam_facelock").unwrap();
        openlog(ident.as_ptr(), 0, LOG_AUTH);
        let msg = std::ffi::CString::new(
            format!("pam_facelock({}): {} for user {}", service, result, user)
        ).unwrap();
...
10. Syslog entries visible in `journalctl -t pam_facelock`

## Verification

```bash
cargo build -p pam-facelock --release
nm -D target/release/libpam_facelock.so | grep pam_sm
stat -c%s target/release/libpam_facelock.so
```
