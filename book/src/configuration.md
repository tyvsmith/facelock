...
| `abort_if_lid_closed` | bool | `true` | Refuse face auth when the laptop lid is closed (camera blocked). The daemon reads the lid from logind, falls back to procfs where procfs can see a lid, and refuses when neither answers. |
