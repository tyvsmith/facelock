...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `mode` | string | `"both"` | Notification mode. `"off"` -- no notifications. `"terminal"` -- PAM text prompts only. `"desktop"` -- desktop popups only (via D-Bus/notify-send). `"both"` -- terminal and desktop. |
...
| `notify_on_failure` | bool | `true` | Notify on failed face match. |
...
Optional TPM 2.0 encryption for face embeddings at rest. Requires the `facelock-tpm` feature. Currently stubs only -- not yet functional.

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `seal_database` | bool | `false` | Encrypt embeddings with a TPM-bound key. |
| `pcr_binding` | bool | `false` | Verify boot integrity via PCR values. |
| `pcr_indices` | list of u32 | `[0, 1, 2, 3, 7]` | PCR registers to check. |
