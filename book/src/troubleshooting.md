...
The first authentication after boot (or after the daemon starts) is slow because ONNX models must be loaded into memory. This is normal. Subsequent auths in daemon mode take ~50ms.
...
With daemon mode, enable socket activation:
```bash
...
1. Check the journal:
   ...
   ```
2. Verify the socket unit is enabled:
   ```bash
   systemctl status facelock-daemon.socket
   systemctl enable --now facelock-daemon.socket
   ```
...
4. Ensure the model directory exists and has correct permissions:
   ...
   sudo chown root:facelock /var/lib/facelock/models
   sudo chmod 750 /var/lib/facelock/models
   ```
...
### PAM module cannot access socket

The socket at `/run/facelock/facelock.sock` is created by the systemd socket unit. Verify:
```bash
ls -la /run/facelock/facelock.sock
systemctl status facelock-daemon.socket
```
