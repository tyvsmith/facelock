...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.services.facelock-daemon = {
      ...
      after = [ "local-fs.target" "facelock-daemon.socket" ];
      requires = [ "facelock-daemon.socket" ];
      serviceConfig = {
        Type = "simple";
    ...
    systemd.sockets.facelock-daemon = {
      description = "Facelock Face Authentication Socket";
      wantedBy = [ "sockets.target" ];
      socketConfig = {
        ListenStream = "/run/facelock/facelock.sock";
        SocketMode = "0660";
        SocketGroup = "facelock";
