...
howdy-rust is a **local biometric authentication system**. The threat model assumes:
...
- **Attacker cannot modify files** in `/etc/howdy/`, `/var/lib/howdy/`, or `/lib/security/`
...
**Limitation**: IR camera detection by format/name is heuristic. Some cameras report YUYV but are actually IR. The `howdy devices` command should display whether each camera is detected as IR.
...
chown -R root:root /var/lib/howdy/models
chmod 755 /var/lib/howdy/models
chmod 644 /var/lib/howdy/models/*.onnx
```
...
```bash
# Database owned by root, readable only by root and howdy group
chown root:howdy /var/lib/howdy/howdy.db
chmod 640 /var/lib/howdy/howdy.db
```
...
// Set ownership
nix::unistd::chown(socket_path, Some(Uid::from_raw(0)), Some(gid_of("howdy")))?;
```
...
fn verify_peer(stream: &UnixStream) -> Result<()> {
    ...
    // Only root (PAM context) and members of howdy group can connect
    if peer_uid != 0 && !is_in_howdy_group(peer_uid) {
        return Err(HowdyError::Daemon("unauthorized connection".into()));
    }
    Ok(())
}
```
...
fn identify(pamh: *mut libc::c_void) -> libc::c_int {
    ...
    // Format: pam_howdy(service): auth result for user
    syslog(LOG_AUTH | severity, "pam_howdy({}): {} for user {}",
