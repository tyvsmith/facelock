...
| `abort_if_lid_closed` | bool | `true` | Refuse face auth when the laptop lid is closed (camera blocked). PAM and one-shot read the lid in the calling process: closed means any `/proc/acpi/button/lid/*/state` reports `closed`, whatever the firmware names the device, and no lid device counts as open. Daemon `Authenticate` reads logind's `LidClosed` instead (`/proc/acpi` is outside its sandbox under systemd), falls back to procfs where procfs can see a lid, and refuses only when neither source can answer — set `false` on a host with neither. Also set `false` for a docked laptop using an external camera with the lid shut. |
