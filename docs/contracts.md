...
| `MANIFEST.json` | `publish` |
  ...
  `libtss2-esys`. `build` compiles in the pinned trixie image `build-deb`
  uses, not on the hosted Ubuntu runner: noble's tpm2-tss 4.0.1 is below the
  4.1.3 `tss-esapi-sys` needs and its just 1.21.0 does not parse the
  justfile. The job attests that image, the attesting set pins it for the
  `build` slot, and the release artifacts contract holds the container
  image, `BUILD_IMAGE`, and the trixie matrix image equal.
